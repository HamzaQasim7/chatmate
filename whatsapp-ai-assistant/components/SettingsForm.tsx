...
import type { Settings, ToneType, ProviderType } from '@/lib/types';
import { TONE_CONFIG, MODEL_CONFIG, PROVIDER_CONFIG, LANGUAGE_CONFIG } from '@/lib/types';
    ...
    <div className="p-5 space-y-6">
      ...
      <div>
        ...
        <div className="flex gap-2">
          {(Object.keys(PROVIDER_CONFIG) as ProviderType[]).map((providerKey) => {
            const config = PROVIDER_CONFIG[providerKey];
            const isSelected = form.provider === providerKey;
            return (
              <button
                key={providerKey}
                onClick={() => setForm({ ...form, provider: providerKey, apiKey: '' })}
                className={`flex-1 flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border-2 text-sm font-semibold transition-all duration-200 ${isSelected
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
        <label className="block text-sm font-bold text-gray-800 mb-2">
          {PROVIDER_CONFIG[form.provider || 'openai'].label} API Key{' '}
        ...
        <input
          ...
          placeholder={PROVIDER_CONFIG[form.provider || 'openai'].placeholder}
        ...
        <div className="mt-2 text-xs flex items-center gap-2">
          {form.apiKey ? (
            <span className="text-green-600 font-semibold flex items-center gap-1">
              <Check size={12} /> Unlimited {PROVIDER_CONFIG[form.provider || 'openai'].label} Usage
            </span>
          ) : (
      ...
      <div>
        ...
        <div className="relative">
          <select
            ...
            className="w-full pl-3 pr-8 py-2.5 bg-white border border-gray-200 rounded-lg text-sm text-gray-700 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-medium"
          >
            {(Object.keys(MODEL_CONFIG) as Array<keyof typeof MODEL_CONFIG>).map((key) => {
              const config = MODEL_CONFIG[key];
              /* Check if unlocked: If it's NOT pro, it's always unlocked. If it IS pro, we need an API key. */
              const isLocked = config.isPro && !form.apiKey;

              return (
                <option key={key} value={key} disabled={isLocked}>
                  {config.label} {isLocked ? '(Pro Key Required)' : ''}
                </option>
              );
            })}
          </select>
      ...
      <div>
        <div className="flex justify-between items-end mb-2">
          <label className="block text-sm font-semibold text-gray-700">Context History</label>
          <span className="text-xs font-medium text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md">
            {form.contextWindow || 5} Messages
          </span>
        </div>
